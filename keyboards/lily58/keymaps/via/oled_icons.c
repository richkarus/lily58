...
static void render_layer_state(uint8_t const state) {
    ...
    // Remapped for this keymap's 3-layer setup: 0=_BASE, 1=_LOWER, 2=_RAISE
    // (func_layer/state==4 unused, no 4th layer here)
    if      (state == 1) oled_write_P(base_layer, false);
    else if (state == 2) oled_write_P(symb_layer, false);
    else if (state == 4) oled_write_P(func_layer, false);
    else                 oled_write_P(numb_layer, false);
}
